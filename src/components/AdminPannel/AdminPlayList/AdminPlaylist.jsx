...
export default function AdminPlaylist({ myPlayList, playListFetch, item,getDatas }) {
  ...
  const createPlayList = (e) => {
    e.preventDefault();
    fetch('http://185.98.138.13:4000/api/v1/playlists', {
      method: 'POST',
...
useEffect(()=> {
  ...
  const updatePlayList = (e) => {
    ...
    fetch(`http://185.98.138.13:4000/api/v1/playlists/${myPlayList[onSelect].id}`, {
      method: 'PUT',
      headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
  ...
  const postSong = () => {
    fetch(`http://185.98.138.13:4000/api/v1/songs/${selectSong}`, {
      method: 'PUT',
      headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
  ...
  const handleChange = (e) => {
    setDeletePlaylistSelect(JSON.parse(e.target.value));
  };

  useEffect(() => {
    fetch(`http://185.98.138.13:4000/api/v1/playlists/${deletePlaylistSelect}`, {
      method: 'GET',
      headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
    })
      .then((res) => res.json())
  ...
  const deleteSong = () => {
    fetch(`http://185.98.138.13:4000/api/v1/playlists/${deletePlaylistSelect.id}/songs/${deleteSongSelect}`, {
      method: 'DELETE',
      ...
      <div className="">
        <div className="bg-white p-4 bg-opacity-10 flex-col shadow-searchbar rounded-lg flex w-80 mx-4">
          ...
          <select
            ...
            {item.map((song, index) => {
              return (
                <option className=" bg-black bg-opacity-80 rounded-xl" value={song.id} key={index}>
                  {song.title}
                </option>
              );
            })}
          ...
          <div className="mt-3 flex flex-col ">
            ...
            <select
              ...
              {myPlayList.map((playList, index) => {
                return (
                  <option className=" bg-black bg-opacity-80 rounded-xl" value={playList.id} key={index}>
                    {playList.title}
                  </option>
                );
              })}
        ...
        <div className="mt-2 bg-white p-4 bg-opacity-10 flex-col shadow-searchbar rounded-lg flex w-80 mx-4">
          ...
          <select
            ...
            name="add-song"
            id="">
            { deletePlaylistSelect &&  deletePlaylistSelect.songs.map((song, index) => {
                return (
                  <option className=" bg-black bg-opacity-80 rounded-xl" value={song.id} key={index}>
                    {song.title}
                  </option>
          ...
          <div className="mt-3 flex flex-col ">
            ...
            <select
              ...
              {myPlayList.map((playList, index) => {
                return (
                  <option className=" bg-black bg-opacity-80 rounded-xl" value={JSON.stringify(playList)} key={index}>
                    {playList.title}
                  </option>
                );
              })}

...
export default function AdminSongs({ item, albums, myPlayList, artists,getDatas }) {
  const { token } = useContext(authContext);
  const [imgUrl, setImgUrl] = useState();
  const [artistId, setArtistId] = useState();
  const handlePictureSubmission = (e) => {
    e.preventDefault();
    fetch(`http://185.98.138.13:4000/api/v1/artists/${artistId}`, {
      method: 'PUT',
    ...
    <div className="flex text-xs text-white">
      <div className="flex">
        ...
        <div className="flex h-full mx-4">
          <UpdateSongs item={item} getDatas={getDatas} />
        </div>
          ...
          <div className="bg-white p-3 bg-opacity-10 flex-col shadow-searchbar rounded-lg flex  w-96 mx-4">
            ...
            <select
              ...
              {artists.map((artist, index) => {
                return (
                  <option className=" bg-black bg-opacity-80 rounded-xl" key={index} value={artist.id}>
                    {artist.name}
                  </option>
                );
              })}

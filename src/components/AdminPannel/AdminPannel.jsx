...
export default function AdminPannel({ item, albums, hideAdmin, artists,getDatas }) {
  ...
  return (
    <div
      ...
      <div className=" mt-2 w-full flex flex-col h-full">
        {isAdminSong && <AdminSongs getDatas={getDatas} artists={artists} albums={albums} myPlayList={myPlayList} item={item} />}
        {isAdminPlayList && <AdminPlaylist getDatas={getDatas} playListFetch={playListFetch} myPlayList={myPlayList} item={item} />}
      </div>
    </div>
  );
}
